...
| T3 | Path traversal / symlink escape out of an authorized workspace | Canonicalized-root comparison, not string-prefix comparison; explicit tests for `..`, symlinks, Windows drive-letter collisions (`C:\project` vs `C:\project-evil`), mixed separators, Unicode — see [`crates/workspace`](../../crates/workspace) |
...
| T6 | Unknown/unregistered tool is executed | `ToolCapabilityResolver` denies by default; there is no "allow unknown tool" fallback anywhere in the policy engine |
