...
KiCad MCP Pro Companion is the trusted local runtime that decides **whether**
...
Companion does not implement KiCad engineering logic (schematic/PCB editing,
ERC/DRC, manufacturing export). That logic lives entirely in kicad-mcp-pro,
which Companion treats as an opaque MCP server reachable over Streamable HTTP
on `127.0.0.1:3334/mcp` (configurable). Companion never bypasses it and never
...
That something is Companion. It is a new, separate component — not a
...
ChatGPT / Claude / Web / Agent
     ...
     KiCad MCP Cloud (future, not in this repo)
             |
       encrypted OUTBOUND connection (initiated by Companion)
             |
             v
+--------------------------------+
...
2. Remote connectivity is always initiated outbound by Companion.
...
will live in a separate private service that Companion talks to only through
