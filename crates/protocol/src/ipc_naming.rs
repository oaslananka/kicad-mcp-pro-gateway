...
//! global name that could collide across two Companion installs pointed at
...
pub fn socket_name(data_dir: &Path) -> String {
    ...
    format!("kicad-mcp-companion-{:x}.sock", hasher.finish())
}
...
mod tests {
    ...
    fn same_data_dir_produces_the_same_name() {
        let path = Path::new("/tmp/companion-data");
        assert_eq!(socket_name(path), socket_name(path));
    }

    #[test]
    fn different_data_dirs_produce_different_names() {
        let a = Path::new("/tmp/companion-data-a");
        let b = Path::new("/tmp/companion-data-b");
